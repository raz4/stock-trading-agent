...
def test(testing_data, model_file, result):
    ...
    result_df = pandas.DataFrame([], columns=['offset', 'open_price', 'action', 'reward'])
    net_reward = 0.0
    ...
    while True:
        ...
        df = pandas.DataFrame([[int(info["offset"]), stock_test_data.open[int(info["offset"])], Actions(action).name, reward]], columns=['offset', 'open_price', 'action', 'reward'])
        print(df)
